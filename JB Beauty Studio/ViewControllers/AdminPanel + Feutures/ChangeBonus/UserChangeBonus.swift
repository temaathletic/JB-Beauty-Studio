...
class UserChangeBonus: UIViewController {
    ...
    private let label: UILabel = {
        let label = UILabel()
        label.text = "Введите баллы для списания"
...
extension UserChangeBonus {
    ...
    @objc func saveBonus() {
        animateView(saveBtn)
        Service.getBonusPointForDecriment(self.stringValue3) { bonusPoint in
            let bonusFromFB = bonusPoint ?? 0
            let textFromTF = self.bonusTextField.text!
            let bonusInt = Int(textFromTF) ?? 0
            let newBonus = bonusFromFB - bonusInt
            print(bonusFromFB)
            print(textFromTF)
            print(bonusInt)
            print(newBonus)
            
            Service.uploadBonusPoint(newBonus: newBonus, uid: self.stringValue3)
        }
