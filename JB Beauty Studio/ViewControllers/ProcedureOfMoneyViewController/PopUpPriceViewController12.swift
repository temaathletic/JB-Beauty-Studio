//
//  PopUpPriceViewController12.swift
//  JB Beauty Studio
//
...
class PopUpPriceViewController12: BottomPopupViewController {
    
    private let labelTitle: UILabel = {
        let text = UILabel()
        text.text = "Коррекция подбородка"
    ...
    private let label: UILabel = {
        let text = UILabel()
        text.text = "Коррекция подбородка-позволяет откорректировать форму подбородка, добавить ему недостающего объема, сделать менее острым или исправить асимметрию."
        text.numberOfLines = 0
    ...
    @objc func addToCart() {
        ...
        vc.createItem(name: ProcedureName.procedureName12, price: Int64(ProcedurePrice.procedurePrice12), quantity: 1)
        dismiss(animated: true)
    }
